...
- 若先验是**零均值高斯** $P(\theta)=\mathcal{N}(\mathbf{0},\tau^2 I)$，则 $\log P(\theta)=-\dfrac{\|\theta\|_2^2}{2\tau^2}+\text{const}$，于是 MAP 等价于 **MLE + L2 正则（权重衰减 weight decay）**：

$$
\hat\theta_{\text{MAP}} = \arg\min_\theta\Big[\text{NLL}(\theta) + \lambda\|\theta\|_2^2\Big], \qquad \lambda=\frac{1}{2\tau^2}
$$

- 若先验是**零均值拉普拉斯**分布 $P(\theta)\propto\exp(-\|\theta\|_1/\tau)$，则 MAP 等价于 **MLE + L1 正则（产生稀疏解）**。
