...
下面这张动画展示了三种范数的**具体计算过程**（以向量 $\mathbf{x}=(3,4)^\top$ 为例）：

![范数计算过程](figs/ch01-norm-calc_anim.svg)

可以看到，同一向量在不同范数定义下有不同的「长度」——这正是「度量方式决定结果」的直观体现。

- $L_2$ 范数是「日常意义的长度」——欧氏距离；
- $L_1$ 范数是「曼哈顿距离」，对离群值不敏感、常诱导稀疏；
- $L_\infty$ 范数只看绝对值最大的那个分量。

这三种范数在几何上是什么意思？下面这张动画在 2D 平面上把它们画了出来：
