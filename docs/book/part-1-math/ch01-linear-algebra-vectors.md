...
**行还是列？ `x @ W` 与 $A\mathbf{x}$ 的约定差异。** 读到这里你可能发现一个看似矛盾的现象：课本里写 $f(\mathbf{x})=A\mathbf{x}$ （矩阵在左、列向量在右），可 LLM 代码里几乎处处是 `x @ W + b` （向量在左、矩阵在右）。到底哪个对？两个都对，差别只在**向量的朝向**：

- **数学约定**：向量默认是**列向量** $\mathbf{x}\in\mathbb{R}^{n\times 1}$ ，变换写成 $A\mathbf{x}$ ，权重 $A$ 形状 $m\times n$ （矩阵在左）；
- **代码约定**：深度学习把**批/序列维放在最前**，每个 token 的向量在张量里是一**行**（ `x` 形状 `[..., n]` ），是行向量。行向量只能左乘矩阵，所以写作 `x @ W` ，权重 `W` 形状 $n\times m$ （矩阵在右）。

两者描述**同一个变换**，只差一次转置。把列向量公式两边转置： $(A\mathbf{x})^\top=\mathbf{x}^\top A^\top$ ，左边正好是一个行向量。对照 `x @ W` 立刻得到：代码里的 `W` 就是数学里的 $A^\top$ 。换句话说，PyTorch 里那个 $n\times m$ 的 `W` ，就是把课本里 $m\times n$ 的 $A$ 转置过来存的——这样批数据 `x @ W` 才能直接相乘，不必每次临时转置。

那为什么不写 `W @ x` ？因为批数据 `x` 的形状是 `[batch, n]` ，而 `W @ x` 要求 $W$ 的列数去匹配 $x$ 的行数 `batch` ，可 `batch` 每次都在变、又跟特征维无关——形状根本对不上。把样本摆成**行**、让 `W` 站在最右边，批维就能自然广播，一次调用算完整批。

> **`torch.nn.Linear` 的细节。** 它内部把权重存成 `[out, in]` （即课本里的 $A$ ），实际计算是 `x @ weight.T + bias` 。所以你看源码时 `weight` 是 `[out, in]` 、手写 `x @ W` 时 `W` 是 `[in, out]` ——别被绕晕，它们是同一个矩阵的两种存法。

下面这张动画把「列向量 $A\mathbf{x}$ 」与「行向量 `x @ W` 」并排演算一遍（同一个变换，两边的数字完全一致），注意底部还顺带画了批处理的情形：

![行列约定与 x@W](figs/ch01-row-col-convention_anim.svg)
