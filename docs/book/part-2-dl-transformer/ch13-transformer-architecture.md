...
$$
\begin{pmatrix} PE_{(pos,2i)} \cr PE_{(pos,2i+1)} \end{pmatrix}  =  \begin{pmatrix} \sin(\omega_i pos) \cr \cos(\omega_i pos) \end{pmatrix}
$$

把位置从 $pos$ 推到 $pos+k$ ，按三角和角公式：

$$
\begin{pmatrix} \sin(\omega_i(pos{+}k)) \cr \cos(\omega_i(pos{+}k)) \end{pmatrix}  =  \underbrace{\begin{pmatrix} \cos(\omega_i k) & \sin(\omega_i k) \cr -\sin(\omega_i k) & \cos(\omega_i k) \end{pmatrix}}_{\text{旋转 }R(\omega_i k)} \cdot \begin{pmatrix} \sin(\omega_i pos) \cr \cos(\omega_i pos) \end{pmatrix}
$$
