...
\boxed{
\begin{aligned}
\text{输入门：}\quad & i_t = \sigma \bigl(W_i [\mathbf{h}_{t-1},\mathbf{x}_t]+\mathbf{b}_i\bigr) \cr
\text{遗忘门：}\quad & f_t = \sigma \bigl(W_f [\mathbf{h}_{t-1},\mathbf{x}_t]+\mathbf{b}_f\bigr) \cr
\text{输出门：}\quad & o_t = \sigma \bigl(W_o [\mathbf{h}_{t-1},\mathbf{x}_t]+\mathbf{b}_o\bigr) \cr
...
\boxed{
\begin{aligned}
\text{细胞状态：}\quad & \mathbf{c}_t = f_t \odot \mathbf{c}_{t-1}  +  i_t \odot \tilde{\mathbf{c}}_t \cr
...
1. **推导题**：设一个 3 步的基础 RNN（无输出头、损失只作用在 $\mathbf{h}_3$ 上），写出 $\frac{\partial L}{\partial \mathbf{h}_1}$ 的完整表达式（含雅可比连乘）。若每个 $`W_h \mathrm{diag}(1-\mathbf{h}_j^2)`$ 的最大奇异值是 $0.5$ ， $\Vert\frac{\partial L}{\partial \mathbf{h}_1}\Vert$ 大约是 $\Vert\frac{\partial L}{\partial \mathbf{h}_3}\Vert$ 的多少倍？这说明什么？
