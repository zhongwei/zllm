...
$$
\mathcal{V}_k^{(t)}=\{\text{概率最大的 }k\text{ 个 token}\},\qquad \tilde\pi_{t,i}=\begin{cases}\pi_{t,i}\big/\!\sum_{j\in\mathcal{V}_k^{(t)}}\pi_{t,j} & i\in\mathcal{V}_k^{(t)}\\ 0 & \text{否则}\end{cases},\quad x_t\sim\tilde{\boldsymbol\pi}_t
$$
...
$$
z'_{t,i}=\begin{cases}z_{t,i}/\lambda & x_i\text{ 已在 }x_{<t}\text{ 中出现过}\\ z_{t,i} & \text{否则}\end{cases},\qquad \lambda>1
$$
